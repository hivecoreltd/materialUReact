...
import { getCategories } from "../Services/Servis"
...
export default function Category_view({ items }) {
      ...
      {views.length && <div className="Cards" style={{ magrinTop: "10px" }}>
        <Grid container justify="center" spacing={2} wrap="wrap">
          {views.map((item1, index) => {
            return (
              <Grid item key={index}>
                <OutlinedCard item={item1} actionpath={`/categroy/detail/${item1._id}`}></OutlinedCard>
              </Grid>
            );
          })}
        </Grid>
